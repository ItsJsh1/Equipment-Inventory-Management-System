...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 dark:text-gray-400">Manage equipment categories</p>
        </div>
    ...
    <div class="card p-4">
        <form method="GET" action="{{ route('categories.index') }}" class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search categories..." class="input-field">
            </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="table-row">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $category->equipment_count ?? 0 }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @can('manage_categories')
                                <button type="button" x-data @click="$dispatch('open-edit-modal', { id: {{ $category->id }}, name: '{{ addslashes($category->name) }}' })" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded" title="Edit">
                    ...
                    @empty
                    <tr><td colspan="3" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No categories found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($categories->hasPages())<div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">{{ $categories->links() }}</div>@endif
    </div>
</div>
...
<!-- Edit Modal -->
<div x-data="{ open: false, item: { id: null, name: '' } }" @open-edit-modal.window="open = true; item = $event.detail" x-show="open" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div x-show="open" x-transition class="fixed inset-0 bg-black/50" @click="open = false"></div>
        <div x-show="open" x-transition class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
