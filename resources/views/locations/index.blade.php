...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 dark:text-gray-400">Manage equipment storage locations</p>
        </div>
    ...
    <div class="card p-4">
        <form method="GET" action="{{ route('locations.index') }}" class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search locations..." class="input-field">
            </div>
            <div class="flex gap-2">
                <button type="submit" class="btn-primary">Search</button>
                <a href="{{ route('locations.index') }}" class="btn-secondary">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="table-row">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $location->address ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $location->equipment_count ?? 0 }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @can('manage_locations')
                                <button type="button" x-data @click="$dispatch('open-edit-modal', { id: {{ $location->id }}, name: '{{ addslashes($location->name) }}', address: '{{ addslashes($location->address ?? '') }}' })" class="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded" title="Edit">
                    ...
                    @empty
                    <tr><td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No locations found</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($locations->hasPages())<div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">{{ $locations->links() }}</div>@endif
    </div>
</div>
...
<div x-data="{ open: false }" @open-create-modal.window="open = true" x-show="open" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div x-show="open" x-transition class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Add Location</h3>
            <form method="POST" action="{{ route('locations.store') }}">
                ...
                <div class="space-y-4">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Address</label><textarea name="address" rows="3" class="input-field" placeholder="Enter location address..."></textarea></div>
                </div>
...
<!-- Edit Modal -->
<div x-data="{ open: false, item: { id: null, name: '', address: '' } }" @open-edit-modal.window="open = true; item = $event.detail" x-show="open" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div x-show="open" x-transition class="fixed inset-0 bg-black/50" @click="open = false"></div>
        <div x-show="open" x-transition class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Edit Location</h3>
            <form :action="'/locations/' + item.id" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Address</label><textarea name="address" rows="3" class="input-field" x-model="item.address" placeholder="Enter location address..."></textarea></div>
                </div>
